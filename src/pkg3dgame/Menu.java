...
public class Menu extends MouseAdapter {
	...
	public Menu(Game game, HUD hud) {
		...
		TraillLifeOp = Config.getShadow();
		Game.master = Config.getVolume();
		Game.Music = Config.getSound();
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (mouseOver(mx, my, (Game.WIDTH / 2) + 90, (Game.HEIGHT / 2) - 75, 30, 27)
				&& game.gameState == STATE.Option) {
			TraillLifeOp += 0.1f;
			Config.setShadow(Config.getShadow() + 0.1f);
		}
		...
		/*
		 * if(mouseOver(mx , my,(Game.WIDTH/2) - 125, (Game.HEIGHT/2) - 160, 250, 64) &&
		 * game.gameState == STATE.Option) { game.gameDificulty = Game.DIFICULTY.FACIL;
		 * 
		 * }
		 * 
		 * if(mouseOver(mx , my,(Game.WIDTH/2) - 125, (Game.HEIGHT/2) - 80, 250, 64) &&
		 * game.gameState == STATE.Option) { game.gameDificulty = Game.DIFICULTY.NORMAL;
		 * 
		 * }
		 * 
		 * if(mouseOver(mx , my,(Game.WIDTH/2) - 125, (Game.HEIGHT/2) , 250, 64 )&&
		 * game.gameState == STATE.Option) { game.gameDificulty =
		 * Game.DIFICULTY.DIFICIL;
		 * 
		 * }
		 * 
		 * if(mouseOver(mx , my,(Game.WIDTH/2) - 125, (Game.HEIGHT/2) + 80, 250, 64) &&
		 * game.gameState == STATE.Option) { game.gameDificulty = Game.DIFICULTY.INSANO;
		 * 
		 * 
		 * }
		 * 
		 * if(mouseOver(mx , my,(Game.WIDTH/2) - 125, (Game.HEIGHT/2) + 160, 250, 64) &&
		 * game.gameState == STATE.Option) { game.gameDificulty = Game.DIFICULTY.BRUTAL;
		 * 
		 * System.out.println("Vai sair?"); }
		 */
		if (mouseOver(mx, my, (Game.WIDTH / 2) + 90, (Game.HEIGHT / 2) + 85, 30, 27)) {
			Game.master += 10;
			Game.master = (int) Main.clamp(Game.master, 0, 90);
			Game.mainSound.setVolume((float) Game.master / 100);
			Config.setVolume(Game.master);
		}
		...
		if (mouseOver(mx, my, (Game.WIDTH / 2) - 125, (Game.HEIGHT / 2) + 80, (Game.HEIGHT / 2) + 20, 64)
				&& game.gameState == STATE.Menu) {
	...
	public void render(Graphics g) {
		...
		if (game.gameState == STATE.Menu) {
			...
			g.drawString("Shadows: " + new DecimalFormat("#.0").format(TraillLifeOp),
					(Game.WIDTH / 2) - 110 + Game.addX, (Game.HEIGHT / 2) - 50 + Game.addY);
			g.drawString("Back", Game.WIDTH / 2 - 55 + Game.addX, Game.HEIGHT - 15 + Game.addY);

			g.drawString("Music: " + Game.Music, (Game.WIDTH / 2) - 110 + Game.addX,
					(Game.HEIGHT / 2) + 30 + Game.addY);
			g.drawString("Volume: " + Game.master, (Game.WIDTH / 2) - 110 + Game.addX,
					(Game.HEIGHT / 2) + 110 + Game.addY);
			...
			/*
			 * if(mouseOverStart) { g.setColor(Color.white); g.drawRect((Game.WIDTH/2) -
			 * 120, (Game.HEIGHT/2) - 75, 240, 54); }
			 */
			if (mouseOverContinue) {
				g.setColor(Color.white);
				g.drawRect((Game.WIDTH / 2) - 120 + Game.addX, (Game.HEIGHT / 2) + 5 + Game.addY, 240, 54);
			...
			/*
			 * if(mouseOverOption) { g.setColor(Color.white); g.drawRect((Game.WIDTH/2) -
			 * 120, (Game.HEIGHT/2) + 85, 240, 54); } if(mouseOverQuit) {
			 * g.setColor(Color.white); g.drawRect((Game.WIDTH/2) - 120, (Game.HEIGHT/2) +
			 * 165, 240, 54); }
			 */
			if (mouseOverBack) {
			...
			g.drawRect((Game.WIDTH / 2) - 125 + Game.addX, (Game.HEIGHT / 2) + Game.addY, 250, 64);
			/*
			 * g.setColor(Color.white); g.drawRect((Game.WIDTH/2) - 125, (Game.HEIGHT/2) ,
			 * 250, 64);
			 * 
			 * g.setColor(Color.white); g.drawRect((Game.WIDTH/2) - 125, (Game.HEIGHT/2) +
			 * 80, 250, 64);
